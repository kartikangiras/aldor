...
import { Wallet, CreditCard, CheckCircle, XCircle, AlertTriangle, Loader2, Shield, Zap, ArrowUpRight } from 'lucide-react';
...
function PaymentCard({
    ...
    return (
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 10, color: '#888888' }}>{payment.agent ?? 'Agent'} payment confirmed on-chain.</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 10, color: '#888888' }}>{payment.error ?? 'You rejected this payment.'}</div>
        </div>
      </div>
    );
  }
  ...
  const handleDodo = async () => {
    ...
    setDodoLoading(true);
    try {
      // Use a fixed $1.00 top-up for Dodo (agent amounts are too small for fiat min)
      ...
      const cancelUrl = typeof window !== 'undefined' ? `${window.location.origin}/` : undefined;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: 13, color: '#00ff94', fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" }}>
          {formatAmount(payment.challenge)}
        </span>
        <span style={{ fontSize: 9, color: '#555555', marginLeft: 'auto' }}>
          {formatAssetLabel(payment.challenge)}
        </span>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            >
              <Wallet size={18} />
              Pay with Wallet
              <span style={{ fontSize: 9, color: walletConnected ? '#00ff94aa' : '#666666', fontWeight: 400, textTransform: 'none' }}>
                SOL / Palm USD
              </span>
            </button>
            <button
            ...
            >
              {dodoLoading ? <Loader2 size={18} className="animate-spin-slow" /> : <CreditCard size={18} />}
              {dodoLoading ? 'Opening…' : 'Pay with Dodo'}
              <span style={{ fontSize: 9, color: '#9d4eddaa', fontWeight: 400, textTransform: 'none' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              style={{
                flex: 1,
                padding: '10px',
                background: walletConnected ? 'rgba(0,255,148,0.1)' : '#333333',
                ...
                textTransform: 'uppercase',
                letterSpacing: '0.05em',
                cursor: walletConnected && !isSigning ? 'pointer' : 'not-allowed',
                display: 'flex',
                alignItems: 'center',
            ...
            >
              {isSigning ? <Loader2 size={14} className="animate-spin-slow" /> : <CheckCircle size={14} />}
          ...
          {dodoUrl ? (
            <>
              ...
              <div style={{ fontSize: 10, color: '#888888', textAlign: 'center' }}>
                After completing payment in Dodo, return here and click <strong>Pay with Wallet</strong> to finish.
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  onClick={() => onApprove(payment.requestId)}
                ...
                >
                  {isSigning ? <Loader2 size={14} className="animate-spin-slow" /> : <Wallet size={14} />}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button
            ...
            style={{ background: 'transparent', border: 'none', color: '#555555', cursor: 'pointer', fontSize: 11 }}
          >
            Dismiss All
          </button>
        </div>
